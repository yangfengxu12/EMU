...
ret=$?
if [ $ret == 0 ]; then 
  command=$cmd" "$prepareimage" pack  -k "$ecckey" -p 1 -r 44 -v "$version" -f "$bin" -t "$sign" "$sfb" -o 192" 
  $command >> $projectdir"/output.txt"
  ret=$?
  if [ $ret == 0 ]; then
    command=$cmd" "$prepareimage" header -k  "$ecckey"  -p 1 -r 44 -v "$version"  -f "$bin" -t "$sign" -o 512 "$headerbin
    $command >> $projectdir"/output.txt"
    ret=$?
    if [ $ret == 0 ]; then
