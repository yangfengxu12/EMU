...
set "command=%python%%prepareimage% trans -k %oemkey% -f SE_ReadKey -s .SE_Key_Data -e 1 -v V7M > %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
goto end
...
set "command=%python%%prepareimage% trans -k %oemkey% -f SE_ReadKey -s .SE_Key_Data -v V7M > %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
set "command=%python%%prepareimage% trans -k %ecckey% -f SE_ReadKey_Pub -e 1 -v V7M >> %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
goto end
...
set "command=%python%%prepareimage% trans -k %ecckey% -f SE_ReadKey_Pub  -s .SE_Key_Data -e 1 -v V7M > %asmfile%"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
goto end
